...
public class RecordedSearchOwner extends Simulation {
  ...
  private ScenarioBuilder scn = scenario("RecordedSearchOwner")
    .exec(
      http("request_0")
        .post("/petclinic/api/owners")
        .headers(headers_0)
        .body(RawFileBody("example/recorder_scenarios/recordedsearchowner/0000_request.json"))
    )
    .pause(14)
    .exec(
      http("request_1")
        .get("/petclinic/api/owners?lastName=Cuong")
        .headers(headers_1)
    );
